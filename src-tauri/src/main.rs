// Cua so duy nhat cua Asrum Launcher.
//
// App tai truc tiep https://evowars.io/ (khong clone/patch source cua trang
// goc), nen khong co index.html rieng de chen <link>/<script> vao truoc.
// Titlebar (markup + CSS + JS trong thu muc titlebar/) duoc nhung thang vao
// binary luc bien dich (include_str!) roi tiem vao trang bang
// initialization_script — chay truoc khi trang duoc parse, xem
// titlebar_injection_script() ben duoi.
//
...
/// lam initialization_script — chay truoc khi trang (evowars.io) duoc
/// parse, nen phai tu cho document.body xuat hien (DOMContentLoaded) roi
/// moi chen <style> + markup titlebar vao, sau do chay tiep logic cua
/// titlebar.js (bind nut, F11/Esc) — luc do cac phan tu da ton tai nen
/// document.getElementById(...) hoat dong binh thuong.
///
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("loi khi khoi dong Asrum Launcher");
}
...
/// bam Publish, bo qua release con o dang Draft).
///
...
/// thoat giua chung — day la gioi han cua Windows installer, khong phai
/// loi. Mac dinh sau khi cai xong Tauri se TU DONG MO LAI app
/// (restart_after_install mac dinh la true) — o day tat no di (false) de
/// ban cap nhat chi co hieu luc o lan nguoi dung tu mo app ke tiep.
