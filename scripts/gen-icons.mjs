// Sinh bo icon Windows tu img/asrum.png bang `tauri icon`.
//
// Duoc goi tu tauri.conf.json (beforeDevCommand / beforeBuildCommand), nen
// ca `npm run tauri dev` lan `npm run tauri build` deu tu chay — khong can
// commit src-tauri/icons/ vao git.
//
// Co cache: luu hash cua asrum.png vao src-tauri/icons/.source-hash. Neu
// hash khong doi va cac file icon can thiet van con, script bo qua, khong
// chay lai.
...
const SOURCE = path.join(ROOT, "img", "asrum.png");
...
function main() {
  if (!fs.existsSync(SOURCE)) {
    fail(
      "khong tim thay img/asrum.png.\n" +
        "Dat logo (PNG vuong, nen 1024x1024, co alpha) vao img/asrum.png roi chay lai."
    );
  }
  ...
  console.log("[gen-icons] dang sinh icon tu img/asrum.png ...");
  ...
  if (missing.length > 0) {
    fail(
      `\`tauri icon\` chay xong nhung thieu file: ${missing.join(", ")}.\n` +
        "Kiem tra lai img/asrum.png co dung la PNG vuong khong."
    );
  }
